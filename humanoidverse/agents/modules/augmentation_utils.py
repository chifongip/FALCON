...
class RobotConfig:
    ...
    symmetry_joint_names = {'left_ankle_pitch_joint': 'right_ankle_pitch_joint',
    ...
    flip_sign_joint_names = ['left_hip_roll_joint',
                             'left_hip_yaw_joint',
                             'right_hip_roll_joint',
                             'right_hip_yaw_joint',
                             'left_ankle_roll_joint',
...
class SymmetryUtils:
    ...
    def __init__(self, env: Any) -> None:
        ...
        self.upper_body_joint_index_map: torch.Tensor = torch.empty(0)
        self.upper_body_sign_flip_mask: torch.Tensor = torch.empty(0)
        self.lower_body_joint_index_map: torch.Tensor = torch.empty(0)
        self.lower_body_sign_flip_mask: torch.Tensor = torch.empty(0)
    ...
    def _init_robot_config(self) -> None:
        ...
        # Order: self.joint_index_map -> dof_names
        # Order: self.sign_flip_mask -> dof_names
        # TODO: Hardcoded
        self.upper_body_joint_index_map = self.joint_index_map[13:]
        self.upper_body_sign_flip_mask = self.sign_flip_mask[13:]
        self.lower_body_joint_index_map = self.joint_index_map[:13]
        self.lower_body_sign_flip_mask = self.sign_flip_mask[:13]
    ...
    def mirror_xz_plane(self, observation: torch.Tensor, env: Any, obs_list: Sequence[str]) -> torch.Tensor:
        ...
        for obs_key in obs_list:
            ...
            mirrored_obs = mirrored_obs_all  # Copy entire obs
            ...
            mirrored_obs_all = mirrored_obs.reshape(B, O)
            # idx += cur_obs_length
    ...
    def mirror_action_xz_plane(self, action: torch.Tensor) -> torch.Tensor:
        ...
        if O == 10:
            # upper body
            upper_body_joint_index_map = self.upper_body_joint_index_map - 13
            return action[..., upper_body_joint_index_map] * self.upper_body_sign_flip_mask
        elif O == 13:
            # lower body
            return action[..., self.lower_body_joint_index_map] * self.lower_body_sign_flip_mask
        else:
    ...
    def mirror_obs_ref_upper_dof_pos(self, ref_upper_dof_pos: torch.Tensor) -> torch.Tensor:
        """Mirrors the reference/target upper-body joint positions.

        This applies left-right symmetry mapping and sign flips to the desired
        upper-body joint positions (typically arms + waist) according to the robot's
        symmetry configuration.

        Parameters
        ----------
        ref_upper_dof_pos : torch.Tensor
            Reference upper-body joint positions in the original DOF order.
            Shape: [..., num_upper_dofs]

        Returns
        -------
        torch.Tensor
            Mirrored reference joint positions: remapped joints with appropriate sign flips.
        """
        upper_body_joint_index_map = self.upper_body_joint_index_map - 13   # TODO: Hardcoded
        return ref_upper_dof_pos[..., upper_body_joint_index_map] * self.upper_body_sign_flip_mask  # TODO: Should I flip upper body?
    ...
    def mirror_obs_left_ee_apply_force(self, left_ee_force: torch.Tensor) -> torch.Tensor:
        """Mirrors the left end-effector applied force when the robot is flipped across x-z plane.

        When the robot is mirrored (left becomes right), the original left EE force becomes the 
        right EE force in the mirrored robot, with the y-component negated due to coordinate flip.

        Parameters
        ----------
        left_ee_force : torch.Tensor
            Left EE applied force in base frame, shape [..., 3] with layout [fx, fy, fz].

        Returns
        -------
        torch.Tensor
            Mirrored force (to be placed in right EE slot): [fx, -fy, fz]
        """
        mirrored = left_ee_force.clone()
        mirrored[..., 1] = -mirrored[..., 1]  # Negate y-component
        return mirrored

    def mirror_obs_right_ee_apply_force(self, right_ee_force: torch.Tensor) -> torch.Tensor:
        """Mirrors the right end-effector applied force when the robot is flipped across x-z plane.

        When the robot is mirrored (right becomes left), the original right EE force becomes the 
        left EE force in the mirrored robot, with the y-component negated.

        Parameters
        ----------
        right_ee_force : torch.Tensor
            Right EE applied force in base frame, shape [..., 3] with layout [fx, fy, fz].

        Returns
        -------
        torch.Tensor
            Mirrored force (to be placed in left EE slot): [fx, -fy, fz]
        """
        mirrored = right_ee_force.clone()
        mirrored[..., 1] = -mirrored[..., 1]  # Negate y-component
        return mirrored
